...
def split_train_test(
    data, n_qubits: int, train_ratio: float = 2 / 3, random: bool = False, seed: int = 0
):
    """
...
def scale_data(
    ...
    test_size: int,
    n_qubits: int,
    scaler_min: float = 0.2,
    scaler_max: float = 0.8,
):
    """
...
def train_model(
    ...
    if loss_function not in valid_loss_functions:
        raise ValueError(
            "Invalid loss function! Only currently supported loss function is 'mean square error'"
        )
    ...
    for i in tqdm(range(epochs)):
        ...
        c = cost(
            weights, x, target_y
        )  # Calculating the cost using the whole train data
    ...
    if bool_plot == True or save_plot != None:
        ...
        plt.figtext(
            x=0,
            y=0,
            s="initial train cost" + str(cst[0]) + "; final train cost:" + str(cst[-1]),
        )
        if save_plot != None:
...
def import_circuit(circuit: str):
    ...
    if circuit == "Ising":
        ...
        raise ValueError(
            "Invalid circuit. Only currently supported circuits are 'MPS' and 'Ising': use one of these or add a circuit to the Circuits library."
        )
