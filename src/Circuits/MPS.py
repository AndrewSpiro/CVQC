...
def initialize_MPS_circuit(
    n_qubits: int = r + 1, seed=0, jax_key=0, bool_test=False, bool_draw=False
):
    """"""
    ...
    def MPS_circuit(weights, x):
        ...
        """
        qml.AngleEmbedding(
            x, wires=range(n_qubits)[:-1]
        )  # Features x are embedded in rotation angles
        qml.MPS(
            wires=range(n_qubits),
            n_block_wires=2,
            block=block,
            n_params_block=n_params_b,
            template_weights=weights,
        )  # Variational layer
        return qml.expval(
            qml.PauliZ(wires=n_qubits - 1)
        )  # Expectation value of the \sigma_z operator on the last qubit
